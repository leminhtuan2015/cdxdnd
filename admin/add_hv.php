<?php 
if (!isset($_SESSION))
{session_start();
}
if(!isset($_SESSION['this_user']) || !isset($_SESSION['this_lever']) || ($_SESSION['this_lever'] != 'admin')){
	header('Location:/admin/index.php');}
include('../config/config.php');
if(isset($_GET['add']) && $_GET['add']='ok'){
$hv_name=isset($_REQUEST['hv_name']) ? ($_REQUEST['hv_name']):"";

$sql_add="INSERT INTO hv(hv_name) VALUE (
'".$hv_name."')";
//echo $sql_add;exit();
$res=exec_update($sql_add);
header('Location:/admin/');
}?>


<?php include('include/header.php');?>

            <div id="center-column">
                <div class="top-bar">
                    <h1>Thêm trình độ học vấn mới</h1>
                </div>
                <div class="table">
                    <form id="form_add_hv" name="form_add_hv" method="POST" action="add_hv.php?add=ok">
						<table class="table_men" width="765" border="0" align="center" cellpadding="10" cellspacing="1">
							<tr>
							  <td width="151">Tên trình độ học vấn</td>
							  <td width="436"><label for="hv_name"></label>
							  <input name="hv_name" type="text" id="hv_name" size="70" /></td>
							</tr>
		
							<tr>
							  <td>Thứ tự hiển thị</td>
							  <td><label for="hv_order"></label>
								<select name="hv_order" id="hv_order">
								<?php for ($stt=-30; $stt<=30; $stt++)
								{
									if ($stt==0){
										echo '<option value="0" selected="selected">0</option>';}
									else
									{
										echo '<option value="'.$stt.'">'.$stt.'</option>';}
								}
								?>
							  </select></td>
							</tr>
							
							<tr>
							  <td colspan="2" align="center"><input type="submit" name="button" id="button" value="Lưu lại" /> 
							  <input type="reset" name="button2" id="button2" value="Nhập lại" /></td>
							</tr>
						</table>
					</form>
					</div>
            </div>
        </div>
<?php include('include/footer.php');?>  